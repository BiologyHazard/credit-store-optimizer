...
def 进程(进程序号, 价格行, 价值行, Kn··, 当前信用矩阵):
    ...
    每日获取信用列表长度 = Kn··.shape[0]
    总价格矩阵 = 价格行[:, None, None]
    # shape = (2 ** 10, 1, 1)
    总价值矩阵 = 价值行[:, None, None]
    # shape = (2 ** 10, 1, 1)
    剩余信用矩阵 = np.maximum(np.minimum(当前信用矩阵 - 总价格矩阵, 300), 0)
    # shape = (2 ** 10, 每日获取信用列表长度, 301)

    目标函数矩阵 = np.where(
        总价格矩阵 <= 当前信用矩阵,
        总价值矩阵 + Kn··[np.arange(每日获取信用列表长度)[None, :, None], 剩余信用矩阵],
        0
    )
    # shape = (2 ** 10, 每日获取信用列表长度, 301)
    目标函数最大值矩阵 = np.amax(目标函数矩阵, axis=0)
    # shape = (每日获取信用列表长度, 301)
    logger.debug(f'{进程序号} 进程结束')
    return 目标函数最大值矩阵


def 多进程迭代计算K···(考虑的天数N, 每日获取信用列表):
    每日获取信用列表长度 = len(每日获取信用列表)
    每日获取信用矩阵 = np.array(每日获取信用列表, dtype=int)[None, :, None]
    # shape = (1, 每日获取信用列表长度, 1)
    继承信用矩阵 = np.arange(301)[None, None, :]
    # shape = (1, 1, 301)
    当前信用矩阵 = 继承信用矩阵 + 每日获取信用矩阵
    # shape = (1, 每日获取信用列表长度, 301)

    K0·· = np.zeros((每日获取信用列表长度, 301))
    K··· = np.empty((考虑的天数N, 每日获取信用列表长度, 301))
    K···[0] = K0··
...
if __name__ == '__main__':
    ...
    考虑的天数N = 11
    每日获取信用列表 = list(range(0, 1001, 5))

    K··· = 多进程迭代计算K···(考虑的天数N, 每日获取信用列表)

    obj = [{} for _ in range(考虑的天数N)]
    for n in range(考虑的天数N):
        for i, 每日获取信用 in enumerate(每日获取信用列表):
            obj[n][str(每日获取信用)] = K···[n, i, :].tolist()  # json 的 key 只能是字符串
