...
class 从左到右购买_设定黑白名单策略(信用交易所购买策略):
    ...
    def 求解(self, 商店: 信用交易所) -> list[bool]:
        ...
        # 先购买白名单中的商品
        for 商品序号, 商品 in 计算未购买的商品列表(商店):
            if 商品.物品名称 in self.白名单 and 商品.现价 <= 当前信用:
                购买指标向量[商品序号] = True
                当前信用 -= 商品.现价
        # 再购买不在黑名单中的商品
...
def 策略字典(每日获得的信用: int):
    return {
        # '啥都不买策略': 啥都不买策略(),
        # '随机购买_直到不能购买为止策略': 随机购买_直到不能购买为止策略(),
        # '从左到右购买_直到不能购买为止策略': 从左到右购买_直到不能购买为止策略(),
        # '从左到右购买_设定黑白名单策略': 从左到右购买_设定黑白名单策略(
        #     黑名单={'加急许可', '家具零件小', '家具零件大', '碳', '碳素'},
        #     白名单={'招聘许可', '龙门币小', '龙门币大', '技巧概要·卷2'},
        #     信用溢出时无视黑名单=False,
        # ),
        # '按性价比购买_直到不能购买为止策略': 按性价比购买_直到不能购买为止策略(),
        '考虑天数为N=11时的最优策略': 考虑天数为N时的最优策略(每日获得的信用=每日获得的信用, N=11),
        # '最优策略的近似策略': 最优策略的近似策略(每日获得的信用=每日获得的信用),
        # '按性价比购买_并设定性价比阈值策略': 按性价比购买_并设定性价比阈值策略(每日获得的信用=每日获得的信用),
    }
...
if __name__ == '__main__':
    ...
    结果 = dict(zip(map(str, 每日获得的信用列表), 结果))
