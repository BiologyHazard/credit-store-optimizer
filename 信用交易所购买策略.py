...
class 按考虑天数为N时的最优策略购买策略(信用交易所购买策略):
...
def 比较策略的优劣(
    ...
    if 商店列表 is None:
        商店列表 = [商店.设为全部未购买() for 商店 in 信用交易所统计字典.values()]*5
        random.shuffle(商店列表)
    return {
...
if __name__ == '__main__':
    ...
    策略字典 = {
        ...
        '按考虑天数为N时的最优策略购买策略': 按考虑天数为N时的最优策略购买策略(每日获得的信用=每日获取信用),
    }
