...
class 最优购买策略的近似策略(考虑天数为N时的信用交易所最优购买策略):
    def __init__(self, 每日获得的信用: int):
        ...
        if 每日获得的信用 < 150:
            ...
            self.K_N减1_C_· = 信用能购买的商品碎片的价值(range(301))
        else:
            # 每日获得的信用大于等于 时，取信用能购买的商品碎片的价值[C - 150 <= c <= C + 150] 作为近似值
            self.K_N减1_C_· = 信用能购买的商品碎片的价值(range(每日获得的信用 - 150, 每日获得的信用 + 151))
...
def 策略字典(每日获得的信用: int):
    ...
    #         信用小于等于300时停止购买=False,
    #     ),
    #     '信用溢出时无视黑名单': 从左到右购买_设定黑白名单策略(
    #         黑名单=黑名单,
    #         白名单=白名单,
    #         信用溢出时无视黑名单=True,
    #         信用小于等于300时停止购买=False,
    #     ),
    #     '信用溢出时停止购买_控信用': 从左到右购买_设定黑白名单策略(
    #         黑名单=黑名单,
    #         白名单=白名单,
    #         信用溢出时无视黑名单=False,
    #         信用小于等于300时停止购买=True,
    #     ),
    #     '信用溢出时无视黑名单_控信用': 从左到右购买_设定黑白名单策略(
    #         黑名单=黑名单,
    #         白名单=白名单,
    #         信用溢出时无视黑名单=True,
    #         信用小于等于300时停止购买=True,
