...
class 最优策略的近似策略(考虑天数为N时的最优策略):
    def __init__(self, 每日获得的信用: int):
        ...
        if 每日获得的信用 < 150:
            self.K_N减1_C_· = [信用能购买的商品碎片的价值[c] for c in range(0, 151)]
        self.K_N减1_C_· = [信用能购买的商品碎片的价值[c] for c in range(每日获得的信用 - 150, 每日获得的信用 + 151)]  # 作为近似值
...
def 模拟一轮购买(
    ...
    当前信用 = 初始继承信用 + 每日获得的信用
    已购买总价值 = 0
    if isinstance(策略, 考虑天数为N时的最优策略) and 顺序 is not None:
        # 快速求解
        for 商店序号 in 顺序:
...
def 策略字典(每日获得的信用: int):
    ...
    return {
        '啥都不买策略': 啥都不买策略(),
        '随机购买_直到不能购买为止策略': 随机购买(信用小于等于300时停止购买=False),
        '随机购买_直到信用小于等于300策略': 随机购买(信用小于等于300时停止购买=True),
        '从左到右购买_直到不能购买为止策略': 从左到右购买(信用小于等于300时停止购买=False),
        '从左到右购买_直到信用小于等于300策略': 从左到右购买(信用小于等于300时停止购买=True),
        '从左到右购买_设定黑白名单策略_信用溢出时停止购买': 从左到右购买_设定黑白名单策略(
            黑名单=黑名单,
            白名单=白名单,
            信用溢出时无视黑名单=False,
            信用小于等于300时停止购买=False,
        ),
        '从左到右购买_设定黑白名单策略_信用溢出时无视黑名单': 从左到右购买_设定黑白名单策略(
            黑名单=黑名单,
            白名单=白名单,
            信用溢出时无视黑名单=True,
            信用小于等于300时停止购买=False,
        ),
        '按性价比从高到低购买_直到不能购买为止策略': 按性价比从高到低购买(信用小于等于300时停止购买=False),
        '按性价比从高到低购买_直到信用小于等于300策略': 按性价比从高到低购买(信用小于等于300时停止购买=True),
        f'考虑天数为N={N}时的最优策略': 考虑天数为N时的最优策略(每日获得的信用=每日获得的信用, N=N),
        '最优策略的近似策略': 最优策略的近似策略(每日获得的信用=每日获得的信用),
        '按性价比从高到低购买_并设定性价比阈值策略': 按性价比从高到低购买_并设定性价比阈值策略(每日获得的信用=每日获得的信用),
    }
