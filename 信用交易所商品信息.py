...
物品价值表路径 = Path("数据/明日方舟一图流-物品价值表-2025-07-07.json")
物品价值表 = json.loads(物品价值表路径.read_text('utf-8'))
物品名称到价值的映射字典 = {item['name']: item['apValue'] for item in 物品价值表} | {
    '家具零件': 0.0001,
    '加急许可': 0.001,
}
信用交易所物品名称列表: list[str] = []
信用交易所商品名称列表: list[str] = []
信用交易所商品价格字典: dict[str, int] = {}
信用交易所商品理智价值字典: dict[str, float] = {}
with open(信用交易所商品信息路径, 'r', encoding='utf-8', newline='') as csvfile:
    ...
    for row in csv_reader:
        ...
        if not row['价格']:  # 跳过 讯使、嘉维尔、坚雷
            continue
        价格 = int(row['价格'])
        信用交易所物品名称列表.append(物品名称)
        信用交易所商品名称列表.append(商品名称)
        信用交易所商品价格字典[商品名称] = 价格
        物品的理智价值 = 物品名称到价值的映射字典[物品名称]
        信用交易所商品理智价值字典[商品名称] = 物品的理智价值 * 数量
